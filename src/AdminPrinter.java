...
public class AdminPrinter extends JFrame{
	MySql db = new MySql();
	JButton back = new JButton("BACK");
	String productList[] = db.printerList();
	JComboBox productBox = new JComboBox(productList);
	JButton add = new JButton("Add Printer");
	JButton remove = new JButton("Remove Printer");
	String selected = null;
	JLabel title = new JLabel("Add or remove products");
	JLabel pro = new JLabel("SELECT A PRINTER");
				...
				new ActionListener(){
					public void actionPerformed(ActionEvent event){
						...
						if(choice == JOptionPane.YES_OPTION){
							String sql = "Delete from `printer_list` where Model = '"+ selected +"'";
							db.removeProduct(sql);
							JOptionPane.showMessageDialog(null,"Product successfully removed!!");
							dispose();
							AdminPrinter AC = new AdminPrinter();
						}
					}
				}
		);
				...
				new ActionListener(){
					public void actionPerformed(ActionEvent event){
						...
						if(choice == JOptionPane.YES_OPTION){
							...
							String sql = "INSERT INTO `printer_list` (`Product ID`, `Model`, `Regular Price`, `Discounted Price`, `Quantity`) VALUES ('"+ realId +"', '"+ mText +"', '"+ realPrice +"', '"+ realDiscountPrice +"', '"+ realQuantity +"')";
							db.addProduct(sql);

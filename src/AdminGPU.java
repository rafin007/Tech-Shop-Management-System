...
public class AdminGPU extends JFrame{
	MySql db = new MySql();
	JButton back = new JButton("BACK");
	String productList[] = db.cpuList();
	JComboBox productBox = new JComboBox(productList);
	JButton add = new JButton("Add Processor");
	JButton remove = new JButton("Remove Processor");
	String selected = null;
	JLabel title = new JLabel("Add or remove products");
	JLabel pro = new JLabel("SELECT A PROCESSOR");
				...
				new ActionListener(){
					public void actionPerformed(ActionEvent event){
						...
						if(choice == JOptionPane.YES_OPTION){
							...
							String sql = "INSERT INTO `gpu_list` (`Product ID`, `Model`, `Regular Price`, `Discounted Price`, `Quantity`) VALUES ('"+ realId +"', '"+ mText +"', '"+ realPrice +"', '"+ realDiscountPrice +"', '"+ realQuantity +"')";
							db.addProduct(sql);

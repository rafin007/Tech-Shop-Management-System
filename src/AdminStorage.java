...
public class AdminStorage extends JFrame{
	MySql db = new MySql();
	JButton back = new JButton("BACK");
	String productList[] = db.storageList();
	JComboBox productBox = new JComboBox(productList);
	JButton add = new JButton("Add Storage");
	JButton remove = new JButton("Remove Storage");
	String selected = null;
	JLabel title = new JLabel("Add or remove products");
	JLabel pro = new JLabel("SELECT A STORAGE");
				...
				new ActionListener(){
					public void actionPerformed(ActionEvent event){
						...
						if(choice == JOptionPane.YES_OPTION){
							String sql = "Delete from `storage_list` where Model = '"+ selected +"'";
							db.removeProduct(sql);
							JOptionPane.showMessageDialog(null,"Product successfully removed!!");
							dispose();
							AdminStorage AS = new AdminStorage();
						}
					}
				}
		);
				...
				new ActionListener(){
					public void actionPerformed(ActionEvent event){
						...
						if(choice == JOptionPane.YES_OPTION){
							...
							String sql = "INSERT INTO `storage_list` (`Product ID`, `Model`, `Regular Price`, `Discounted Price`, `Quantity`) VALUES ('"+ realId +"', '"+ mText +"', '"+ realPrice +"', '"+ realDiscountPrice +"', '"+ realQuantity +"')";
							db.addProduct(sql);

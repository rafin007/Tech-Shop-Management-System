...
public class AdminIO extends JFrame{
	MySql db = new MySql();
	JButton back = new JButton("BACK");
	String productList[] = db.IOList();
	JComboBox productBox = new JComboBox(productList);
	JButton add = new JButton("Add Device");
	JButton remove = new JButton("Remove Device");
	String selected = null;
	JLabel title = new JLabel("Add or remove products");
	JLabel pro = new JLabel("SELECT A DEVICE");
				...
				new ActionListener(){
					public void actionPerformed(ActionEvent event){
						...
						if(choice == JOptionPane.YES_OPTION){
							...
							String sql = "INSERT INTO `io_list` (`Product ID`, `Model`, `Regular Price`, `Discounted Price`, `Quantity`) VALUES ('"+ realId +"', '"+ mText +"', '"+ realPrice +"', '"+ realDiscountPrice +"', '"+ realQuantity +"')";
							db.addProduct(sql);
